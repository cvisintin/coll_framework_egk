plot.gbm2 <- function (x, i.var = 1, n.trees = x$n.trees, continuous.resolution = 100, 
          return.grid = FALSE, type = "link", xlab, ylab, ...) 
{
  ...
  if (length(i.var) == 1) {
    if (!f.factor) {
      ...
      if (x$distribution$name == "multinomial") {
        ...
        plot(range(X$X1), range(X$y), type = "n", 
             ylab = ylab, xlab = xlab, ...)
        for (ii in 1:x$num.classes) {
          lines(X$X1, X$y[, ii], xlab = xlab, 
                ylab = ylab, col = ii, ...)
        }
      }
      else if (is.element(x$distribution$name, c("bernoulli", 
                                                 "pairwise"))) {
        plot(X$X1, X$y, type = "l", 
             ylab = ylab, xlab = xlab, ...)
      }
...
plot.gbm2(kang.brt, i.var=3, type="response", ylab="KANGAROO OCCURRENCE", xlab="LIGHT", yaxt="n", ylim=c(0,0.25), xaxt="n", bty="n", lwd=0.5)
axis(1, lwd = 0.2)
axis(2, lwd = 0.2)
box(lwd=0.3)
dev.off()
...
par(mgp=c(2.0,0.5,0),mar=c(3.5,3.5,1,1), cex.axis=0.8)
plot.gbm2(kang.brt, i.var=1, type="response", ylab="KANGAROO OCCURRENCE", xlab="ELEV", yaxt="n", ylim=c(0,0.07), xaxt="n", bty="n", lwd=0.5)
axis(1, lwd = 0.2)
axis(2, lwd = 0.2)
box(lwd=0.3)
dev.off()
...
par(mgp=c(2.0,0.5,0),mar=c(3.5,3.5,1,1), cex.axis=0.8)
plot.gbm2(kang.brt, i.var=5, type="response", ylab="KANGAROO OCCURRENCE", xlab="PRECDM", yaxt="n", ylim=c(0,0.1), xaxt="n", bty="n", lwd=0.5)
axis(1, lwd = 0.2)
axis(2, lwd = 0.2)
box(lwd=0.3)
dev.off()
